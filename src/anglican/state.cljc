...
(defn push-catch
  ...
  state"
  [state tag cont]
  (update state ::catch-stack conj [tag cont]))
...
(defn current-catch-cont
  "returns current catch continuation of the catch stack"
  [state]
  (let [[[_ cont] & _] (::catch-stack state)] 
    cont))
...
(defn pop-catch-until-tag
  ...
  matches the tag"
  [state tag]
  (if (seq (::catch-stack state))
    (let [[[t _] & _] (::catch-stack state)]
      (if (= t tag)
        state
        (recur (pop-catch state) tag)))
    ;; empty stack
    (push-catch state 
                ::uncaught 
                (fn [value _]
