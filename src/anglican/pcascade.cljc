  ...
  (:use ;; TODO David [anglican.state :exclude [initial-state]]
        #?(:clj anglican.inference
          :cljs [anglican.inference :only [infer exec checkpoint
                                           checkpoint-id rand]])
        [anglican.runtime :only [exp log ceil log-sum-exp observe* sample*]]))
          ...
          (swap! (@(state ::average-weights) observe-id)
                 (fn [[log-total cnt]]
                   [(log-sum-exp log-total
                                 (+ log-weight (log mplier)))
                    (+ cnt mplier)]))]
      (if (= log-total (/ -1. 0.)) 1.    ; all particles had 0 weight
          (exp (- log-weight (- log-total (log cnt))))))))
...
#?(:clj
  (defmethod checkpoint [::algorithm anglican.trap.observe] [_ obs]
    (let [;; Incorporate new observation
          ...
          multiplexer (let [ceil-ratio (ceil weight-ratio)]
                        (if (< (- ceil-ratio weight-ratio) (rand))
                          ceil-ratio (- ceil-ratio 1.)))
          log-weight (- log-weight (log weight-ratio))]
