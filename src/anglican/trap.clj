...
(defn cps-of-try
  ...
  delimits code in which values can be thrown using `throw`"
  [args cont]
  (let [cont-pop-name (*gensym* "CP")
        cont-pop-value-name (*gensym* "V")
        cont-pop `(~'fn ~cont-pop-name [~cont-pop-value-name ~'$state] ~(continue cont cont-pop-value-name `(pop-try-cont ~'$state)))
        push-cont `(push-try-cont ~'$state ~cont)]
    `(~'let [~'$state ~push-cont]
            ~(cps-of-do args cont-pop))))
...
(defn cps-of-throw
  ...
  (make-of-args args
                (fn [args*]
                  (continue `(peek-try-cont ~'$state) (first args*) `(pop-try-cont ~'$state)))))
